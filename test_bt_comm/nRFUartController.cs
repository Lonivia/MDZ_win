...
namespace nRFUart
{
    ...
    public class nRFUartController
    {
        ...
        public void SendBytes(byte[]bt)
        {
            ...
            //    AddToLog("Max packet size is 20 characters, text is truncated.");
            //}
            //byte[] bts = { 0x1, 0x2, 0x3 };
            masterEmulator.SendData(pipeSetup.UartRxPipe, bt);

            //string decodedString = Encoding.UTF8.GetString(encodedBytes);
            AddToLog(string.Format("TX: {0}", bt.ToString()));
        }
        ...
        public void conn_WATCH() {
            if (true)
            {
                //if (dgvDeviceDiscovery.SelectedRows.Count > 0)
                {
                    ...
                    {
                        //this.Cursor = Cursors.WaitCursor;
                        BtDeviceAddress address = new BtDeviceAddress(Watch_address);
